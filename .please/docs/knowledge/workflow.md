...
> Development workflow conventions for infer-please.
...
bun test src/some.test.ts   # Run specific test file
```

### Linting & Formatting

```bash
turbo run lint              # Lint all packages (via turbo)
turbo run lint:fix          # Auto-fix lint + format all packages
eslint .                    # Lint single package (from package dir)
eslint . --fix              # Auto-fix single package
```
...
- Mock external dependencies (TEI processes, llama-server processes)
- Test both success and failure cases

### Integration Testing

- Test complete request flows (client → server → backend proxy)
- Verify process lifecycle management (spawn, health check, idle timeout)
