...
- [ ] FR-1: Hono 기반 HTTP 서버가 단일 포트(기본 :3141)에서 아래 엔드포인트를 노출한다.
  ...
  - `POST /v1/embeddings`
  - `POST /v1/rerank`
- [ ] FR-2: `/v1/models` 및 `/v1/models/{model}`은 `infer-please.yaml`의 정적 모델 레지스트리를 OpenAI `Model` 스키마 (`{id, object:'model', created, owned_by}`)로 반환한다.
- [ ] FR-3: `POST /v1/embeddings`는 OpenAI 포맷 (`{model, input, encoding_format?, dimensions?}`) 요청을 받아 `@infer-please/tei` `TeiManager`로 라우팅하고 OpenAI `Embedding` 응답 (`{object:'list', data:[{embedding, index, object:'embedding'}], model, usage}`)을 반환한다. 문자열 및 배열 input 모두 지원.
...
- [ ] FR-7: 설정 파일(`infer-please.yaml`) 스키마를 확장한다. 최소 필드:
...
- [ ] FR-11: `bun infer-please start` 명령으로 서버를 시작한다 (기존 CLI 스켈레톤 확장).

### Non-functional Requirements

- [ ] NFR-1: 라우트 핸들러는 하위 레이어(@infer-please/tei 등)만 의존하며 `Bun.spawn()`을 직접 호출하지 않는다 (ARCHITECTURE.md 불변항 준수).
...
- [ ] AC-6: `bun infer-please start --port 3141` 명령으로 서버 시작과 엔드포인트 접근 가능.
...
- Hot-reload of `infer-please.yaml` (매 재시작 필요)
- Multi-tenant API key 관리 (단일 토큰만 지원)

## Assumptions

- `@infer-please/tei` `TeiManager`는 이미 embedding/rerank API를 제공하며 이번 트랙에서 그대로 재사용한다 (변경 불필요).
- 설정 파일 이름은 `infer-please.yaml` 고정. 로더/파서는 이번 트랙에서 구현 (아직 없음).
